...
- **状态**: ✅ codex done
- **内容**: 用户反馈批量上传壁纸时每张都要单独点授权，要求点击一次同意后即可上传整批作品
- **Codex 完成**:
  - 上传队列的授权确认从单项状态改为整批状态，一次勾选即可覆盖当前批次全部图片 / 视频
  - 发布校验只逐项检查标题，授权改为统一检查整批确认
  - 每张作品提交到 API 时仍写入 `licenseAccepted: true` 与当前授权版本，保留原有合规记录
  - 队列清空时自动取消本次整批授权，避免下一批误沿用
- **验证**:
  - `pnpm type-check`
  - `pnpm lint`

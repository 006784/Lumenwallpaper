...
### Google Cloud Vision / Tasks

可选接入 Google Cloud Vision 作为上传风控与标签兜底：

- `SAFE_SEARCH_DETECTION` 会写入 `safety_*` 字段；高风险新上传会从 `published` 自动转为 `processing`，进入人工审核路径
- `LABEL_DETECTION` 会在 Gemini 等生成式 provider 全部失败时作为 AI 标签兜底，避免图片长期停留在 `failed`

Vision 可以使用 API key，也可以复用 service account：
...
Cloud Tasks 用于把长任务排进 Google 队列，目前 OpenClaw 提供 `POST /api/openclaw/tasks`，可异步触发批量 backfill 或 reanalyze：

```env
GOOGLE_CLOUD_TASKS_LOCATION=asia-east1
GOOGLE_CLOUD_TASKS_QUEUE=lumen-wallpaper-jobs
GOOGLE_CLOUD_TASKS_TARGET_BASE_URL=https://byteify.icu
GOOGLE_CLOUD_TASKS_SERVICE_ACCOUNT_EMAIL=
```
